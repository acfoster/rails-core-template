...
class UserTest < ActiveSupport::TestCase
  ...
  test "password must be at least 12 characters" do
    user = User.new(email: "test@example.com", password: "Pass1234567")
    assert_not user.valid?
    assert_includes user.errors[:password], "is too short (minimum is 12 characters)"
  end

  test "password must meet complexity when 12-15 characters" do
    user = User.new(email: "test@example.com", password: "passwordonly")
    assert_not user.valid?
    assert_includes user.errors[:password], "must be at least 16 characters, or at least 12 characters with a mix of upper, lower, number, and symbol"
  end

  test "valid password with mixed categories is accepted" do
    user = User.new(email: "test@example.com", password: "Password123!")
    user.skip_confirmation! # Skip confirmation for this test
    assert user.valid?
  end

  test "password with 16 characters is accepted without complexity" do
    user = User.new(email: "test@example.com", password: "longpasswordonly")
    user.skip_confirmation! # Skip confirmation for this test
    assert user.valid?
  end
