class Log < ApplicationRecord
  ...
  def self.without_async
    previous = Thread.current[:log_async_disabled]
    Thread.current[:log_async_disabled] = true
    yield
  ensure
    Thread.current[:log_async_disabled] = previous
  end
  ...
  def self.log(log_type:, level:, message:, user: nil, user_id: nil, action: nil, controller: nil, request_id: nil, ip_address: nil, context: {}, metadata: {})
    ...
    if Thread.current[:log_async_disabled]
      return create!(attrs)
    end
